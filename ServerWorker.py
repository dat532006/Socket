...
class ServerWorker:
	...
	def processRtspRequest(self, data):
		...
		# Process TEARDOWN request
			...
			# 'event'/'rtpSocket' only exist if PLAY was issued before
			if 'event' in self.clientInfo:
				self.clientInfo['event'].set()
			...
			if data: 
				...
				try:
					...
					while(curPos < dataLen): # send data in chunks
						endPos = min(curPos + MAX_PAYLOAD, dataLen)
						dataChunk = data[curPos:endPos]
						if(endPos >= dataLen): # last chunk
							marker = 1
						else:
							marker = 0
						self.clientInfo['rtpSocket'].sendto(self.makeRtp(dataChunk, frameNumber, marker),(address,port))
						curPos = endPos
				except:
	...
	def makeRtp(self, payload, frameNbr, marker=0): # marker default to 0
